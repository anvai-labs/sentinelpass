...
export async function loadRegistryBadgeCount() {
    try {
        const result = await invoke('get_registry_overview', {
...
function updateRegistryBadge(count) {
...
// Rendering
// ---------------------------------------------------------------------------
function renderRegistryDashboard(data) {
...
function renderStatusStrip(posture) {
...
function renderFindingsList(posture) {
    ...
    container.innerHTML = posture
        ...
        <div class="registry-finding-row" data-entry-id="${entry.entry_id}">
            <div class="registry-finding-row-header" data-action="open-entry">
                <div class="registry-finding-title">${escapeHtml(entry.title)}${entry.entity_name
        ? ` <span class="metadata-muted">&middot; ${escapeHtml(entry.entity_name)}</span>`
        : ''}</div>
                <span class="status-badge status-${entry.status.replace(/_/g, '-')}">${STATUS_LABELS[entry.status]}</span>
            </div>
            ...
            ${entry.status === 'reused'
        ? `<button class="registry-disclosure-toggle" data-action="toggle-cluster" data-entry-id="${entry.entry_id}">shared with ${entry.reuse_count - 1} other${entry.reuse_count - 1 === 1 ? '' : 's'} &rarr;</button>
    ...
    container.querySelectorAll('[data-action="open-entry"]').forEach((el) => {
        el.addEventListener('click', () => {
            const entryId = parseInt(el.closest('.registry-finding-row').dataset.entryId);
...
function toggleReuseDisclosure(entryId) {
    if (!overview) {
        return;
    }
    const membersContainer = document.querySelector(`[data-cluster-for="${entryId}"]`);
    if (!membersContainer) {
        return;
    }
...
async function openEntityFromPosture(entryId) {
...
function renderEntitiesList(entities) {
    ...
    container.innerHTML = entities
        ...
        <div class="registry-entity-row">
            ...
            <div class="registry-finding-reasons">
                criticality=${escapeHtml(summary.entity.criticality)} &middot; ${summary.credential_count} credential${summary.credential_count === 1 ? '' : 's'}
            </div>
        </div>
    `)
        .join('');
