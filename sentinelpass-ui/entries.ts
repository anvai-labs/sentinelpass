...
import {
    ...
    entries, setEntries,
    currentTotpMetadata, setCurrentTotpMetadata,
    currentFilter, entriesRefreshInFlight, setEntriesRefreshInFlight,
 ...
 */
export async function loadEntriesWithOptions({
    preserveSelection = true,
    silent = false,
    selectionMissingToast = false
} = {}) {
...
export async function refreshEntriesNow() {
    ...
    const refreshBtn = document.getElementById('refresh-entries-btn') as HTMLButtonElement | null;
    if (refreshBtn) {
 ...
 */
export function renderEntryList(filteredEntries: any[] | null = null) {
    ...
    document.querySelectorAll('.entry-item').forEach(item => {
        item.addEventListener('click', () => loadEntry(parseInt((item as HTMLElement).dataset.id)));
    });
}
 ...
 */
export async function loadEntry(entryId: number) {
    try {
        ...
        document.querySelectorAll('.entry-item').forEach(item => {
            item.classList.toggle('active', parseInt((item as HTMLElement).dataset.id) === entryId);
        });
        ...
        // Populate form
        (document.getElementById('detail-title') as HTMLInputElement).value = entry.title;
        (document.getElementById('detail-username') as HTMLInputElement).value = entry.username;
...
export function createNewEntry() {
    ...
    // Clear form
    (document.getElementById('detail-title') as HTMLInputElement).value = '';
    (document.getElementById('detail-username') as HTMLInputElement).value = '';
    ...
    (document.getElementById('detail-title') as HTMLInputElement).focus();
}
