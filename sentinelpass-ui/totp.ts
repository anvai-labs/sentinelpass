 ...
 */
export function normalizeTotpMetadata(metadata: any): any {
 ...
 */
export function setTotpButtonState(hasSelection: boolean, configured: boolean) {
    const copyButton = document.getElementById('copy-totp') as HTMLButtonElement | null;
    const configureButton = document.getElementById('configure-totp') as HTMLButtonElement | null;
    const removeButton = document.getElementById('remove-totp') as HTMLButtonElement | null;
 ...
 */
export async function updateTotpAvailability(entryId: any) {
    if (!entryId) {
...
export function openTotpModal() {
    ...
    const uriInput = document.getElementById('totp-otpauth-uri') as HTMLInputElement | null;
    const secretInput = document.getElementById('totp-secret') as HTMLInputElement | null;
    const algorithmInput = document.getElementById('totp-algorithm') as HTMLInputElement | null;
    const digitsInput = document.getElementById('totp-digits') as HTMLInputElement | null;
    const periodInput = document.getElementById('totp-period') as HTMLInputElement | null;
    const issuerInput = document.getElementById('totp-issuer') as HTMLInputElement | null;
    const accountInput = document.getElementById('totp-account-name') as HTMLInputElement | null;
 ...
 */
export async function saveTotpForEntry(event: Event) {
    ...
    const uri = (document.getElementById('totp-otpauth-uri') as HTMLInputElement).value.trim();
    const secret = (document.getElementById('totp-secret') as HTMLInputElement).value.trim();
    const algorithm = (document.getElementById('totp-algorithm') as HTMLInputElement).value;
    const digits = Number((document.getElementById('totp-digits') as HTMLInputElement).value);
    const period = Number((document.getElementById('totp-period') as HTMLInputElement).value);
    const issuer = (document.getElementById('totp-issuer') as HTMLInputElement).value.trim();
    const accountName = (document.getElementById('totp-account-name') as HTMLInputElement).value.trim();
