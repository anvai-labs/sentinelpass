...
**Session transport (WBS-509/510):** clients negotiate a SECURED session (SessionHello/SessionAccept; every envelope sealed). A first frame that is not a SessionHello — the legacy PLAIN envelope session, bearer token in cleartext — is REFUSED unless the operator sets `SENTINELPASS_ALLOW_PLAIN_IPC=1` (announced per connection; removed in 1.0, same discipline as the other legacy windows below).
