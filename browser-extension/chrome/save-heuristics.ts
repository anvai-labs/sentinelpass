 ...
 */
function hostFromParsedUrl(parsed: URL): string | null {
...
function stripPolicyWwwSuffix(host: string): string | null {
...
/**
 * How a credential URL's transport classifies for save-flow warnings
 * (WBS-706 HTTP warn half).
 *
 * Only an explicitly `http:` URL counts as insecure; a scheme-less value is
 * `unknown` (no scheme is fabricated just to claim the page is secure), and
 * any non-web scheme (`ftp:`, `chrome-extension:`, …) is likewise `unknown`.
 */
export type CredentialUrlSecurity = 'secure' | 'insecure' | 'unknown';

export function classifyCredentialUrlSecurity(rawUrl: unknown): CredentialUrlSecurity {
...
export function domainMatchesPolicy(domain: string, policyDomain: string): boolean {
  ...
  return (
    normalizedDomain === normalizedPolicy ||
 ...
 */
export function autofillPageUrlForDaemon(
  requestedPageUrl: unknown,
  senderUrl: unknown,
  trustedPopup: boolean
): string | null {
  if (!trustedPopup) {
