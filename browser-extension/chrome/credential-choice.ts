...
/** One candidate as listed by the daemon (never includes secrets). */
...
export type CredentialChoice =
  | { action: 'none' }
  | { action: 'fill'; username: string }
  | { action: 'choose'; candidates: CredentialCandidate[] };
 ...
 */
export function decideCredentialChoice(
  ...
  if (requestedUsername) {
    ...
    const match = candidates.find(
      (candidate) => candidate.username.trim().toLowerCase() === wanted
    );
    if (match) {
