...
| Daemon IPC transport | SECURED session required by default (SessionHello handshake; every envelope sealed). The legacy PLAIN envelope session — which would carry the bearer token in cleartext — is refused unless the operator sets `SENTINELPASS_ALLOW_PLAIN_IPC=1` (announced per connection; removed in 1.0, same discipline as the browser-surface legacy windows). WBS-911 F3. | Unix: socket dir 0700 + peer-eUID check, so the gated window is defense-in-depth there; Windows adds AES-256-GCM framing. |
