...
| Passkey Support State | Partial | `sentinelpass-core/src/vault/mod.rs`; `sentinelpass-core/src/database/schema.rs`; `sentinelpass-core/src/database/migrations.rs`; `sentinelpass-core/src/sync/models.rs`; `docs/PASSKEY_PRODUCT_DESIGN.md` | `sentinelpass-core/src/vault/tests.rs`; `sentinelpass-core/src/database/migrations.rs`; `sentinelpass-core/src/sync/models.rs`; `sentinelpass-core/src/lib.rs` | `passkey_reference` is represented as metadata/reference only. SentinelPass does not currently act as a WebAuthn authenticator or credential provider and must not claim custody of passkey private keys. | Implement metadata-only UI/CLI surfaces behind the design constraints; keep raw passkey private material out of generic vault entries unless a platform-authenticator-compatible architecture is approved. |
