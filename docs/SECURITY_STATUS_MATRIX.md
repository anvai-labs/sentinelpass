...
|---------|--------|------------------|----------------------------------|--------|
| Local Argon2id password KDF and random salt | Partial | `sentinelpass-core/src/crypto/kdf.rs`; core tests | Strong baseline and minimums exist; hostile parameters have no hard maximums, intermediate buffers need a zeroization sweep, and mobile calibration is absent | 0.9 |
| AES-256-GCM encrypted entry fields | Partial | `sentinelpass-core/src/crypto/cipher.rs`; vault crypto tests | Ciphertext authenticity exists, but field/record/vault/purpose context is not bound with AAD | 0.9 / ADR-005 |
| Durable encrypted format/versioning | Planned | Schema versioning and legacy `bincode` blobs | Needs self-describing bounded envelope v2, algorithm IDs, golden vectors, and fail-closed compatibility | 0.9 / ADR-005 |
