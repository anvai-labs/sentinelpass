...
| Security-critical key material drawn from `thread_rng()` | TD-SEC-09 | — (WBS-909 adversarial review) | CM | 0.11 | Closed (evidence) — TD row flipped Closed same-change 2026-09-13; adversarial review round 2 extended the fix to three missed sites before merge | All production secret/key generation sourced from `rand::rngs::OsRng`: `crypto/password.rs:148,275`, `vault/recovery.rs:92` (false doc comment now true), `sync/device.rs:21`, `sync/pairing.rs:20,94,107` (incl. the 256-bit v2 pairing ROOT secret), `biometric_hello.rs:112,194` (Hello/mobile slot challenges); every remaining `thread_rng`/`rand::random` hit re-classified test-only (`sync/auth.rs`:141, `vault/recovery.rs`:226, `crypto/kdf.rs`:308 test modules). Verified: fmt + clippy `-D warnings` + full workspace tests + `--features sync` green |
