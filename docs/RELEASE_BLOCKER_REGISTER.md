...
|---|---|---|---|---|---|---|
| Local fields lack semantic AAD | TD-SEC-01 | 303/304/404 | CM | 0.9 | Open (TD row still reads `Open; ADR-005 Proposed` at this baseline — reconciliation gap: the closure work is merged, so this row needs a same-change TD flip; see evidence) | Merged and verifiable in-tree: typed AAD builder `crypto/aad.rs` with golden vectors (WBS-303); SPENV v2 envelopes for all durable classes via `vault/envelope_ops.rs` (WBS-304); post-unlock v1→v2 sweep + `format_version=2` activation gate `vault/activation_ops.rs` (WBS-404/405/406, PRs #102/#104/#107); ADR-005 **Accepted rev 4** |
...
| Epoch does not revoke sync authority | TD-SEC-05 | 312/314/614 | CM | 0.11 | Open (TD row; substantial enforcement has landed — closing requires the TD row's remaining outcome, stale device/slot rejection everywhere) | TD row reads Open (its ADR references still say "Proposed"; ADR-004 is Accepted rev 5 and ADR-006 Accepted rev 2 — TD row update pending). Matrix row "Sync device epoch/revocation": v2 relay rejects mutations below the vault's forward-only epoch high-water (bounded jump); clients dead-letter pulled mutations below their local vault epoch (WBS-614); relay-side epoch advance remains client-asserted (bounded) |
...
| Mobile placeholder security functions | TD-SEC-08 | 104 (labels) / 807 | ME | 0.8.x/0.12 | In progress | TD row, In Progress — WBS-807 Done 2026-09-11 (CloudKit/Drive file-sync placeholders + DriveService removed; biometric placeholder JNI trimmed in WBS-802); remaining scaffolds tracked by WBS-812/813/821/825; ADR-009 Accepted rev 2. Note: the TD-MOB table already records WBS-812/813/821/825 as Closed 2026-09-12 — flip TD-SEC-08 same-change to finish this row |
| Security-critical key material drawn from `thread_rng()` | TD-SEC-09 | — (new row, WBS-909 adversarial review) | CM | 0.11 | Open | TD row (opened 2026-09-12 during the WBS-909 exception review): `crypto/password.rs:148,275` (password/passphrase generation), `vault/recovery.rs:91,289` (recovery keys), `sync/device.rs:21` (device-identity signing secret) use `rand::thread_rng()` instead of drawing from `rand::rngs::OsRng` directly; also fix the false "from the OS CSPRNG" doc comment on `RecoveryKey::generate`. ThreadRng is CSPRNG-reseeded, so this is hardening, not an active vulnerability |
...
| Release smoke tests lack real unlock/round-trip/restore | TD-REL-05 | 905 | Open (TD row) — drill scripts land with this register update; installed-artifact smoke + release-gate wiring remain | `scripts/drills/` (recovery, backup-restore, compromise-rotation), wired into `.github/workflows/drills.yml` (tag + dispatch) |
