...
| Security-critical key material drawn from `thread_rng()` | TD-SEC-09 | — (WBS-909 adversarial review) | CM | 0.11 | Closed (evidence) — TD row flipped Closed same-change 2026-09-13; adversarial review round 2 extended the fix to three missed sites before merge | All production secret/key generation sourced from `rand::rngs::OsRng`: `crypto/password.rs:148,275`, `vault/recovery.rs:92` (false doc comment now true), `sync/device.rs:21`, `sync/pairing.rs:20,94,107` (incl. the 256-bit v2 pairing ROOT secret), `biometric_hello.rs:113,195` (Hello/mobile slot challenges); every remaining `thread_rng`/`rand::random` hit re-classified test-only (`sync/auth.rs`:141, `vault/recovery.rs`:226, `crypto/kdf.rs`:308 test modules). Verified: fmt + clippy `-D warnings` + full workspace tests + `--features sync` green |
...
|---|---|---|---|---|
| Arbitrary HTTP relay URLs accepted | TD-NET-02 | 103 / 617 | Closed (evidence) — TD row flipped Closed same-change 2026-09-13 | `sync/config.rs::validate_relay_url` enforces HTTPS / loopback-only HTTP (`SENTINELPASS_ALLOW_LOOPBACK_RELAY=1`) / no userinfo at init and client construction, with negative tests; matrix row "Relay transport policy" records the v2 client's bounded same-origin redirect policy (max 3 hops, TLS-downgrade and cross-origin refusals, target re-validation — WBS-617) as Implemented; refusal paths `sync/client.rs:511/517`, tests `:557`/`:586` |
