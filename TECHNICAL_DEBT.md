...
Last updated: 2026-09-12 (0.11 cycle — Phase 7 release assurance)
...
| 2026-09-12 | v0.11 (WBS-901/903/908 + TD-NET-06) | Phase 7 stages A2/A3: tag-time security gates (WBS-901 — release.yml's `release` and `crates-publish` jobs now require the tag-time RustSec + npm audits; PR runs don't double-run them; TD-REL-01 → Partial); fuzz targets (WBS-903 — `fuzz/` crate with 4 targets over the real parsers: envelope open, sync v2 mutation parse both ends, IPC frame decode, import/export byte-level parse; relay split into lib+bin so its validators are linkable; `parse_json_import_bytes`/`parse_csv_import_bytes` extracted; scheduled nightly fuzz workflow, not a required check; smoke runs crash-free); TD-NET-06 folded claim pinned at three layers (base64 decode-ratio arithmetic, no-smaller-cap shape check, signed oversized-body router refusal); SBOM (WBS-908 — `cargo auditable build` on release binaries + `cargo audit bin` verification gate; TD-REL-03 → Partial). Adversarial review round 1 (A1) caught a falsified register row (thread_rng in production key-material generation) → TD-SEC-09 opened | worktree branch |
| 2026-09-13 | 0.11 (WBS-902 + WBS-905 sub-item) | TD-REL-01 matrix half: release-gating `feature-matrix` job (3 OS × 3 feature legs, red leg aborts the release; android.yml/ios.yml gain `workflow_dispatch` for manual tag-time mobile runs; `docs/OSS_RELEASE_CHECKLIST.md` §Tag-time CI prerequisites) + WBS-905 sidecar staging fix (cross-platform, fail-closed `build-native-installers.sh`; native-installer-smoke stages sidecars, verifies them in the built bundles, smoke-runs packaged binaries — portable-archive execution smoke only, functional unlock/round-trip remains TD-REL-05) | worktree branch |
| 2026-09-13 | v0.11 (TD-SEC-09) | RNG provenance hardening: all production secret/key generation sourced from `rand::rngs::OsRng` (password/passphrase generator, 256-bit recovery key + doc-comment truthing, device-identity Ed25519 seed, pairing code + HKDF salt + the 256-bit v2 pairing root secret, biometric Hello/mobile-slot challenges); the integration adversarial review caught three production sites the first pass missed (develop-only code absent from the implementer's tree) — extended before merge; post-fix sweep confirmed every remaining ThreadRng/random hit is test-only; TD-SEC-09 → Closed | worktree branch |
| 2026-09-13 | 0.11 (WBS-905 drills + register reconciliation + WBS-911 brief) | Drill scripts `scripts/drills/` (recovery 21/0, backup-restore 35/0, compromise-rotation 15/0 — locally verified on macOS with fail-closed negatives: old-password rejection, recovery-key single-use, restore without `--allow-replace` refused, epoch advance 1→2) + `.github/workflows/drills.yml` (tag + dispatch, 90-day evidence artifacts, Gate unaffected); `docs/RELEASE_BLOCKER_REGISTER.md` reconciled against the TD table (P0/P1 rows refreshed; the audit surfaced stale TD rows TD-SEC-01/05/08 → flipped same-change); `docs/SECURITY_REVIEW_BRIEF.md` commissioning draft (WBS-911) | #138 |
| 2026-09-13 | 0.11 (TD-NET-02) | Register audit follow-up: the v2 client redirect half (WBS-617) had landed but the TD row still read "client redirect rules remain" — flipped Closed with the refusal-path/test citations (`sync/client.rs:511/517`, tests `:557`/`:586`); register P1 row mirrored | docs branch |
...
|----|-----|---------------|------------------|--------|--------|
| TD-SEC-01 | Local encrypted fields lack semantic AAD | `crypto/cipher.rs`; `vault/mod.rs` | Envelope v2 binds vault/entry/purpose/type/epoch/version | 0.9 | Closed 2026-09-13 (reconciliation — work merged via PRs #102/#104/#107, row was left stale at "Open; ADR-005 Proposed" while ADR-005 reached Accepted rev 4): envelope v2 authenticated context binds vault UUID + object identity + purpose + type + envelope-schema + crypto version + epoch + tombstone on every durable seal/open (`crypto/aad.rs` `AadContextBuilder` WBS-303 + `crypto/envelope.rs`, adopted by `vault/envelope_ops.rs` for all durable classes WBS-304); `open_envelope` takes the expected context from the trusted row so cross-row/blob relocation is refused structurally (tested end-to-end); post-unlock v1→v2 sweep + `format_version=2` activation gate (`vault/activation_ops.rs`, WBS-404/405/406); epoch-AAD binding pinned by `epoch_aad_binds_the_wrap_against_db_rollback`; golden vectors + multi-round adversarial review |
...
| TD-SEC-05 | Key epoch does not revoke normal sync/device authority | pairing bootstrap vs normal sync | Epoch on every request/object; stale device/slot rejection | 0.11 | Partial (reconciled 2026-09-13 — row was stale at "Open; ADR-004/006 Proposed" although both ADRs are Accepted; matrix row "Sync device epoch/revocation" is authoritative): LANDED — device keys/revocation checked on every request (Ed25519 middleware); v2 relay rejects mutations below the vault's forward-only epoch high-water (bounded jump); clients dead-letter pulled mutations below their local vault epoch (WBS-614); password rotation revokes ongoing stale-key sync at both ends. REMAINS — relay-side epoch advance is client-asserted (bounded jump), authenticated epoch publication is a later-stage option; revocation notification to remaining devices deferred (ADR-004 rev 5) |
...
| TD-SEC-08 | Android/iOS security functions are incomplete but user-facing scaffolds exist | mobile bridge and native apps | Prototype labeling now; no release-reachable placeholders later | 0.8.x/0.12 | Closed 2026-09-13 (Phase 6, PR #136): no release-reachable placeholder scaffolds remain — real platform keystore slots replace the in-process biometric map (WBS-812 Android Keystore auth-bound RSA-PKCS1 + signature-KDF wrap; WBS-821 iOS Keychain biometryCurrentSet slot), real AutofillService (WBS-813), lifecycle/backup hardening (WBS-814-816), orphan CloudKit/Drive placeholders removed (WBS-807); ADR-009 Accepted rev 2. Feature residuals (not placeholders): iOS slot ENROLLMENT deferred pending a core export surface; device-runtime matrix evidence (WBS-818) |
| TD-SEC-09 | Security-critical key material is generated with `rand::thread_rng()` (ThreadRng) instead of OsRng | `crypto/password.rs:148,275` (password/passphrase generation), `vault/recovery.rs:91,289` (recovery keys), `sync/device.rs:21` (device-identity signing secret) | Key material draws from `rand::rngs::OsRng` directly (ThreadRng kept only for non-secret uses); also fix the false "from the OS CSPRNG" doc comment on `RecoveryKey::generate` | 0.11 | Closed 2026-09-13 (adversarial review round 2 extended the fix to three sites the first pass missed — the v2 pairing root secret and the biometric Hello challenges live in develop-only code absent from the first pass's tree): ALL production secret/key generation now sourced from `rand::rngs::OsRng` — `crypto/password.rs:148,275` (password/passphrase), `vault/recovery.rs:92` (256-bit recovery key; the previously false "OS CSPRNG" doc comment is now true and explicit), `sync/device.rs:21` (Ed25519 identity seed), `sync/pairing.rs:20,94,107` (pairing code, HKDF salt, and the 256-bit v2 pairing ROOT secret itself), `biometric_hello.rs:113,195` (fresh sign-at-release challenge + the seal challenge under the signature-KDF wrap). Post-fix sweep: every remaining `thread_rng`/`rand::random` hit is test-only (`sync/auth.rs` tests at :141, `vault/recovery.rs` tests at :226, `crypto/kdf.rs` tests at :308). Idiom matches the existing `crypto/kdf.rs` OsRng usage; no dependency changes. Verified: fmt + clippy `-D warnings` + `cargo test --workspace` + `--features sync` all green; targeted pairing + biometric_hello suites green after the extension |
...
| TD-NET-02 | Clients accept arbitrary HTTP relay URLs | TLS except explicit loopback development; safe redirects/userinfo rules | 0.8.x/0.11 | Closed 2026-09-13 (both halves landed): `validate_relay_url` (sync/config.rs) enforces HTTPS / loopback-only HTTP (`SENTINELPASS_ALLOW_LOOPBACK_RELAY=1`) / no userinfo at init AND client construction, with negative tests; the v2 client redirect policy is bounded, cross-origin-refusing, and TLS-downgrade-refusing with target re-validation (sync/client.rs custom `Policy::custom`, refusal paths at :511/:517; tests `redirect_policy_refuses_cross_origin_and_downgrades` :557 and `redirect_policy_is_bounded` :586 — WBS-617, matrix row "Relay transport policy": Implemented) |
...
| ID | Gap | Required outcome | Target | Status |
|----|-----|------------------|--------|--------|
| TD-MOB-01 | Android CI omits JNI and JNI-enabled Rust currently fails | Build every Android ABI with JNI and fail on symbol/signature mismatch | 0.12 | Closed 2026-09-11 (WBS-810 + WBS-811): JNI feature compiles/tests/clippy-clean (baseline was 3× E0308 in deleted drive.rs); android.yml builds arm64-v8a/armeabi-v7a/x86_64 with --features jni and fails closed on a 17-symbol llvm-nm check (VaultBridge surface) + stale-symbol guard; build-android.sh adds --features jni + the symbol check on its NDK-r29 ABI set (2 of 3 ABIs; local check warns-and-skips without llvm-nm vs CI fail-closed) |
...
| TD-MOB-03 | Android biometric/sync/autofill contain placeholders | Keystore-bound slot, real sync v2, complete AutofillService | 0.12 | Closed 2026-09-12 (WBS-807/812/813): sync placeholders deleted, Keystore-bound slot shipped (ABI v2), real AutofillService (fill+save via the bridge, domain-matched datasets, AUTH-dataset unlock flow, 39 unit tests green). Residual: relay sync v2 itself is not yet wired on mobile (separate WBS) |
| TD-MOB-04 | Android update is delete-then-add and handle lifecycle can leak | Atomic update and deterministic destroy | 0.12 | Closed 2026-09-11 (WBS-807 + WBS-804): atomic sp_entry_update/nativeUpdateEntry export wired through VaultBridge.updateEntry into VaultState (delete-then-add removed); deterministic destroy with refused double-destroy/use-after-destroy enforced by integration tests |
| TD-MOB-05 | Android lifecycle/privacy/network/backup policy is incomplete | Lock/privacy cover, cleartext deny, verified backup allowlist/no-backup | 0.12 | Closed 2026-09-12 (WBS-814/815/816): registered Application + ProcessLifecycleOwner auto-lock (fixed a mid-session fire bug), FLAG_SECURE cover + lock-screen return, cleartextTrafficPermitted=false, backup/extraction rules exclude vault db sidecars + slot blob (documented allowlist semantics) |
| TD-MOB-06 | iOS biometric state is process-local and unlock is unimplemented | Keychain access-control platform slot surviving restart | 0.12 | In Progress — WBS-821 Done 2026-09-11: DEK stored in Keychain under kSecAccessControlBiometryCurrentSet (survives restart, OS-gated release) via KeychainSlot.swift + sp_slot_open_with_dek; wiring into the consolidating Swift bridge lands with WBS-820 |
| TD-MOB-07 | iOS lacks scene lock/privacy cover and safe pasteboard behavior | Lifecycle lock/cover and local-only expiring pasteboard | 0.12 | Closed 2026-09-12 (WBS-823/824): scenePhase privacy cover + 5-min background auto-lock; all three secret-copy call sites use 30 s expiring pasteboard (Universal-Clipboard absence-of-opt-out documented) |
| TD-MOB-08 | iOS export/delete/backup/Credential Provider are incomplete | Authenticated backup/restore and Credential Provider | 0.12 | Closed 2026-09-12 (WBS-825/826/827): Credential Provider extension (real unlock+filter+ASPasswordCredential flow, App Group), plaintext SwiftData model removed, ADR-008 .spbackup backup/restore through the bridge (BackupService + sp_backup_*) |
| TD-MOB-09 | Duplicate Swift bridges and FFI buffers lack one proven ownership/zeroization contract | Generated ABI, one bridge, panic containment, zeroizing free | 0.12 | Closed 2026-09-12 (WBS-801/804/805/820): generated+drift-checked ABI, 8-rule ownership contract, panic containment, and the orphan duplicate bridge + committed xcframework binaries DELETED; the surviving Swift bridge was fixed against the v2 contract (it double-freed list buffers and leaked the lock handle — never compiled before) |
| TD-MOB-10 | Mobile CI does not demonstrate functional native behavior | Simulator/device unlock/CRUD/update/lock/process-death/autofill tests | 0.12 | Closed 2026-09-12 (WBS-818/828): real androidTest sources (bridge handshake/CRUD/lock/slot) on the fail-closed API 29/34 emulator matrix with all-ABI JNI .so artifacts; real bridge-contract XCTests on the iOS simulator with fail-closed app+extension build gates (old job swallowed failures) |

### P1 -- Release and supply chain

| ID | Gap | Required outcome | Target | Status |
|----|-----|------------------|--------|--------|
| TD-REL-01 | Tag release is not directly gated by all security workflows/features | Required security and feature-matrix dependencies | 1.0 RC | Partial (WBS-901 2026-09-12; WBS-902 + WBS-905 sidecar staging 2026-09-13): release.yml's `release` job `needs:` the tag-time security audits (RustSec under the governed `.cargo/audit.toml` policy + both npm audits) AND the new tag-gated `feature-matrix` job (`cargo build --release --locked --workspace` × {default, `--no-default-features`, `--features sync`} × {linux, macos, windows}; `.github/workflows/release.yml` `feature-matrix`, required by the release job — a red leg aborts the release before anything publishes; is-release-gated so the Gate is unaffected). The `crates-publish` job `needs:` the same security audits but NOT the feature matrix (it publishes only the protocol crate, which has no platform/feature surface). Mobile stays gated by android.yml/ios.yml, now manually dispatchable at a tag ref (`workflow_dispatch` added; checklist step in `docs/OSS_RELEASE_CHECKLIST.md` §Tag-time CI prerequisites). WBS-905: tagged Tauri installers now stage daemon/host sidecars via `scripts/build-native-installers.sh` (Windows .exe handled; fail-closed on empty/missing sidecars) and `native-installer-smoke` verifies sidecars in the built bundles (.deb payload / .app walk) and smoke-runs the packaged CLI `--version` against the tag. REMAINS: signing/notarization gates (WBS-906/907) |
...
| TD-REL-03 | No published SBOM/provenance assurance | Signed SBOM and provenance attestations | 1.0 RC | Partial (WBS-908, 2026-09-12): release binaries embed their dependency SBOM via `cargo auditable build` (offline-verifiable with `cargo audit bin`, drift-proof by construction — rationale + verification gate in release.yml and `docs/OSS_RELEASE_CHECKLIST.md` §Supply chain); REMAINS: signed SBOM publication and provenance attestations (ADR-010, rides WBS-906/907 signing) |
...
| TD-REL-05 | Release smoke tests do not perform a real unlock/round-trip/restore | Installed-artifact functional and recovery smoke suites | 1.0 RC | Partial (2026-09-13): the packaged portable archive is now extracted on each release platform with presence/executability asserted for all four shipped binaries and a real `--version` execution + tag-version match on the CLI (release.yml `native-installer-smoke` "Smoke packaged binaries"); REMAINS: functional unlock/round-trip/restore smoke against installed artifacts |
