...
| 2026-09-07 | v0.10 (WBS-405/406/407/414/415) | 0.10 cycle: full-vault verify-before-activation pass + atomic `format_version=2` activation with downgrade block (405/406); per-version schema fixture ladder from released dumps (407); opaque audit identifiers (414); HMAC-chained audit trail with rotation/retention/public verifier + tamper negatives, `AuditKeyLease` lifecycle (415). Gate-review fix commits: Windows directory-stat (`len()==0` for dirs), dead-letter terminal state, legacy-trigger neutralization, splice detection, born-0600 grants, pair-join canonical KDF | #107 |
| 2026-09-08 | v0.10 (WBS-409/411/410) | Write-path correctness: echo-trigger removal (schema v9; the trigger also corrupted applied sync_version on lagging rows), explicit local/remote write paths with exhaustive enumeration; transactional units of work + SQLite-authorizer fault-injection harness (complete-old/complete-new at every statement); typed NULL end to end incl. legacy `X''` un-bricking; registry sweep self-heal via `mark_backfill_needed`. Integration adversarial review clean; TD-ROB-02/03 closed | #121 |
| 2026-09-08 | deps | Dependabot queue resolved: merged npm-dev group, 17-dep cargo minor/patch group, tower-http 0.6, p256 phantom-dep removal + ignore rules (hkdf>=0.13, windows>=0.62 documented as ecosystem-blocked) | #108,#109,#110,#114 |
| 2026-09-07 | docs | ADR-006…010 adversarial review round 1 (5-reviewer fanout): all ACCEPT-WITH-AMENDMENTS, 2 blockers found and folded (ADR-008 restore×sync self-revert/wedge; ADR-010 nonexistent-updater clause); rev-2 ACCEPTED by owner (#122): ADR-006..010 now gate-open for Phases 2-7 | -- |
...
|----|-----|---------------|------------------|--------|--------|
| TD-SEC-01 | Local encrypted fields lack semantic AAD | `crypto/cipher.rs`; `vault/mod.rs` | Envelope v2 binds vault/entry/purpose/type/epoch/version | 0.9 | Done (2026-09-07): typed AAD builder `crypto/aad.rs` (WBS-303, golden-vector pinned); SPENV v2 envelopes for ALL durable object classes via `vault/envelope_ops.rs` (WBS-304); post-unlock v1→v2 sweep with verify-before-write (WBS-404, PR #104); format_version=2 activation gate (WBS-405/406, PR #107); ADR-005 Accepted rev 4 |
...
| TD-SEC-05 | Key epoch does not revoke normal sync/device authority | pairing bootstrap vs normal sync | Epoch on every request/object; stale device/slot rejection | 0.11 | Open; ADR-004 Accepted rev 5; ADR-006 Accepted rev 2 (WBS-612/614) |
| TD-SEC-06 | Browser IPC authority relies on a self-asserted origin; originless remains allowed | `protocol/envelope.rs`; daemon IPC server | Native-host-specific capability; deny originless | 0.8.x/0.10 | Containment half done (0.8.x): originless browser-surface requests denied by default with `SENTINELPASS_ALLOW_LEGACY_ORIGINLESS` escape hatch + unit tests; capability model open — ADR-007 Accepted rev 2 (WBS-504/505) |
