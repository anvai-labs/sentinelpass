...
| TD-CLIENT-04 | Windows biometric consent is separate from generic keyring retrieval | Cryptographically Windows Hello/protection-bound slot | 0.10 | Closed 2026-09-10 (WBS-710): the stored value is a Hello-BOUND wrap — DEK sealed under HKDF of a TPM/Hello KeyCredential signature over a per-vault challenge; the at-rest keyring entry alone cannot yield the DEK; release requires a fresh Hello-gated private-key operation whose signature regenerates the wrap key (GCM-authenticated; refused gesture/wrong key/tampered blob fail closed). Determinism self-check at enable; legacy enrollments migrate on re-enable. Evidence: 10-case platform-free unit suite + windows-0.61 x86_64-pc-windows-msvc type-check of the KeyCredentialManager surface |
