...
| TD-SEC-09 | Security-critical key material is generated with `rand::thread_rng()` (ThreadRng) instead of OsRng | `crypto/password.rs:148,275` (password/passphrase generation), `vault/recovery.rs:91,289` (recovery keys), `sync/device.rs:21` (device-identity signing secret) | Key material draws from `rand::rngs::OsRng` directly (ThreadRng kept only for non-secret uses); also fix the false "from the OS CSPRNG" doc comment on `RecoveryKey::generate` | 0.11 | Closed 2026-09-13: ALL production secret/key generation now sourced from `rand::rngs::OsRng` — `crypto/password.rs:148,275` (password/passphrase), `vault/recovery.rs:92` (256-bit recovery key; the previously false "OS CSPRNG" doc comment is now true and explicit), `sync/device.rs:21` (Ed25519 identity seed), `sync/pairing.rs:20,94` (pairing code + HKDF salt; the pairing secret is HKDF-derived from code+salt and inherits the provenance). Repo-wide sweep re-classified every remaining `thread_rng`/`rand::random` hit as test-only (`sync/auth.rs` tests, `vault/recovery.rs` tests, `crypto/kdf.rs` tests). Idiom matches the existing `crypto/kdf.rs` OsRng usage; no dependency changes. Verified: fmt + clippy `-D warnings` + `cargo test --workspace` + `--features sync` all green |
