...
// Per-entry payload cap: the configured request-body limit (one consistent
...
mod tests {
    ...
    async fn push_v2_fault_injection_is_all_or_nothing() {
        ...
        let assert_complete_old = |state: &RelayAppState, vault_id: &str, device_id: &Uuid| {
            ...
            let (log, objects, results, counter, device_seq, epoch): (
                ...
                .query_row(
                    "SELECT                          (SELECT COUNT(*) FROM sync_mutations_v2),                          (SELECT COUNT(*) FROM sync_entries_v2),                          (SELECT COUNT(*) FROM mutation_results),                          (SELECT COALESCE((SELECT current_sequence FROM sequence_counters WHERE vault_id = ?1), 0)),                          (SELECT COALESCE((SELECT last_sequence FROM device_sequences WHERE device_id = ?2), 0)),                          (SELECT COALESCE((SELECT key_epoch FROM vault_epochs WHERE vault_id = ?1), 0))",
    ...
    async fn migration_claim_is_one_per_origin() {
        ...
        // Scaffolding exists for the fresh vault. (Guard scoped — the next
        // handler call re-acquires the storage mutex.)
        let counter: i64 = {
            let conn = state.storage.conn().unwrap();
            conn.query_row(
                "SELECT current_sequence FROM sequence_counters WHERE vault_id = ?1",
...
pub async fn migration_claim(
    ...
    // The claimant must be a registered device.
    tx.query_row(
